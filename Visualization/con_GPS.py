...
def process_data():
    try:
        ...
        if len(sensorValues) == 13:
            ...
            sensorValue12.append(float(sensorValues[11]))
            sensorValue13.append(float(sensorValues[12]))
...
def update():
    ...
    if len(timestamps) > 0:
        ...
        new_data9 = dict(x=[sensorValue11[-1]], y=[sensorValue12])
        ...
        source_map.stream(new_data9, rollover=200)

        new_data_table1 = dict(x=[sensorValue12[-1]], y=[sensorValue13[-1]])
        new_data_table2 = dict(x=[sensorValue12[-1]], y=[sensorValue13[-1]])
